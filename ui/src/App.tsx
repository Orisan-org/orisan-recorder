...
import { api, type Banner, type ScanResult, type Status, type UiEvent } from './api.js';
...
function Sessions({ onOpen }: { onOpen: () => void }): React.JSX.Element {
  const [rows, setRows] = useState<{ id: string; startedAt: string; endedAt: string; events: number; flagged: number }[]>([]);
  useEffect(() => { void api.sessions().then((r) => setRows(r.sessions)); }, []);
  return (
    <>
      <h1>Sessions</h1>
      <p className="sub">Recorded sessions, newest first.</p>
      {rows.length === 0 ? (
        <div className="empty"><p>Nothing recorded yet.</p></div>
      ) : (
        <table>
          <thead><tr><th>Session</th><th>Started</th><th>Ended</th><th>Events</th><th>Flagged</th><th /></tr></thead>
          <tbody>
            {rows.map((s) => (
              <tr key={s.id}>
                ...
                <td>{s.events}</td>
                <td>{s.flagged}</td>
                <td><button className="btn" onClick={onOpen}>Open</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function Timeline({ banner }: { banner: Banner | null }): React.JSX.Element {
  const [events, setEvents] = useState<UiEvent[]>([]);
  useEffect(() => { void api.events().then((r) => setEvents(r.events)); }, []);

  return (
    <>
      <h1>Timeline</h1>
      <p className="sub">Every recorded action, in order.</p>
      {banner && <IntegrityBanner banner={banner} />}
      {events.length === 0 ? (
        <div className="empty"><p>No events recorded yet.</p></div>
      ) : (
        <table>
  ...
  return (
    <div className="shell">
      ...
      <main className="main">
        ...
        {screen === 'sessions' && <Sessions onOpen={() => setScreen('timeline')} />}
        {screen === 'timeline' && <Timeline banner={status?.banner ?? null} />}
